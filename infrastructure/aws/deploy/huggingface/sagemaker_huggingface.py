...
from domain.inference import DeploymentStrategy
...
class SagemakerHuggingfaceStrategy(DeploymentStrategy):
    ...
    def deploy(
        ...
        logger.info(
            f"Deployment parameters: nb of replicas: {os.getenv("COPIES")}, nb of gpus:{os.getenv("GPUS")}, instance_type:{os.getenv("GPU_INSTANCE_TYPE")}"
        )
        try:
